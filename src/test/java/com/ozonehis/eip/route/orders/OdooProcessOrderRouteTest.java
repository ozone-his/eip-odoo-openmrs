...
import com.ozonehis.eip.route.OdooTestConstants;
...
public class OdooProcessOrderRouteTest extends BaseOrderOdooRouteTest {
	...
	public void setup() throws Exception {
		...
		advise(ROUTE_ID, new AdviceWithRouteBuilder() {
			...
			public void configure() {
				interceptSendToEndpoint(OdooTestConstants.URI_GET_ENTITY_BY_UUID).skipSendToOriginalEndpoint().to(mockFetchResourceEndpoint);
	...
	public void shouldFailIfThereIsMultipleDraftQuotationsForThePatient() throws Exception {
		...
		mockPatientHandlerEndpoint.whenAnyExchangeReceived(e -> e.setProperty(OdooTestConstants.EX_PROP_ODOO_PATIENT_ID, odooPatientId));
	...
	public void shouldProcessANewOrderForAPatientWithNoExistingQuote() throws Exception {
		...
		mockManageQuoteEndpoint.expectedPropertyReceived(OdooTestConstants.EX_PROP_ODOO_OP, OdooTestConstants.ODOO_OP_CREATE);
	...
	public void shouldProcessQuantityDetailsAndDosingInstructionsForANewDrugOrder() throws Exception {
		...
		final String description = drugName + " | " + qty + " " + tabs + " | " + dose.toString() + " " + mg  + " - " + daily + " - " + duration + " " + week;
	...
	public void shouldProcessQuantityDetailsAndDosingInstructionsForANewDrugOrderAppedingOrdererToDescription() throws Exception {
		...
		final String description = drugName + " | " + qty + " " + tabs + " | " + dose.toString() + " " + mg  + " - " + daily + " - " + duration + " " + week + " | " + "Orderer: " + ((String) ordererResource.get("display")).split(" - ")[1];
		assertEquals(description, exchange.getProperty(EX_PROP_DESC));
	}
	
	@Test
	public void shouldProcessQuantityDetailsAndDosingInstructionsWithAdditionalInstructionsForANewDrugOrder() throws Exception {
		final Integer odooPatientId = 5;
		Map patientResource = new HashMap();
		patientResource.put("uuid", OdooTestConstants.PATIENT_UUID);
		Map orderResource = new HashMap();
		...
		assertEquals(quoteId, exchange.getProperty(EX_PROP_QUOTE_ID));
		assertEquals(unitsId, exchange.getProperty(EX_PROP_UNITS_ID));
		final String description = drugName + " | " + qty + " " + tabs + " | " + dose.toString() + " " + mg  + " - " + daily + " - " + duration + " " + week + " - " + "Follow these additional instructions";
		assertEquals(description, exchange.getProperty(EX_PROP_DESC));
	}
	
	@Test
	public void shouldProcessQuantityDetailsAndDosingInstructionsForANewDrugOrderAppedingCommentToFulfillerToDescription() throws Exception {
		final Integer odooPatientId = 5;
		Map patientResource = new HashMap();
		patientResource.put("uuid", OdooTestConstants.PATIENT_UUID);
		Map orderResource = new HashMap();
		...
		assertEquals(quoteId, exchange.getProperty(EX_PROP_QUOTE_ID));
		assertEquals(unitsId, exchange.getProperty(EX_PROP_UNITS_ID));
		final String description = drugName + " | " + qty + " " + tabs + " | " + dose.toString() + " " + mg  + " - " + daily + " - " + duration + " " + week + " | " + "Notes: " + orderResource.get("commentToFulfiller") + " | " + "Orderer: " + ((String) ordererResource.get("display")).split(" - ")[1];
		assertEquals(description, exchange.getProperty(EX_PROP_DESC));
	}
	...
	public void shouldAppendCommentToFulfillerToDescription() throws Exception {
		...
		assertEquals(0, exchange.getProperty(EX_PROP_ORDER_LINE_COUNT));
		assertEquals(quoteId, exchange.getProperty(EX_PROP_QUOTE_ID));
		final String description = orderName + " | "  + "Notes: " + orderResource.get("commentToFulfiller") + " | " + "Orderer: " + ((String) ordererResource.get("display")).split(" - ")[1];
		assertEquals(description, exchange.getProperty(EX_PROP_DESC));
	}
	...
	@Test
	public void shouldProcessQuantityDetailsAndDosingInstructionsForARevisionDrugOrderAppedingCommentToFulfillerToDescription() throws Exception {
		final Integer odooPatientId = 5;
		Map patientResource = new HashMap();
		patientResource.put("uuid", OdooTestConstants.PATIENT_UUID);
		Map orderResource = new HashMap();
	...
	public void shouldFailIfTheQuantityUnitsAreMappedToMultipleOdooUnits() throws Exception {
		...
		mockFetchResourceEndpoint.whenAnyExchangeReceived(e -> e.getIn().setBody(patientJson));
		exchange.setProperty(EX_PROP_IS_NEW, true);
		exchange.setProperty(OdooTestConstants.EX_PROP_TABLE_RESOURCE_MAP, singletonMap("patient", "patientRepository"));
		exchange.setProperty(EX_PROP_IS_DRUG_ORDER, true);
		mockPatientHandlerEndpoint.expectedMessageCount(1);
		mockPatientHandlerEndpoint.expectedPropertyReceived(EX_PROP_PATIENT, patientResource);
		mockPatientHandlerEndpoint.whenAnyExchangeReceived(e -> e.setProperty(OdooTestConstants.EX_PROP_ODOO_PATIENT_ID, odooPatientId));
