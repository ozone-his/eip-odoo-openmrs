...
import static com.ozonehis.eip.odooopenmrs.route.OdooTestConstants.ODOO_OP_CREATE;
import static com.ozonehis.eip.odooopenmrs.route.OdooTestConstants.ODOO_RPC_METHOD;
import static com.ozonehis.eip.odooopenmrs.route.OdooTestConstants.URI_SAVE_CALENDAR_EVENT;
...
public class SaveCalendarEventInOdooRouteTest extends BaseOdooApiRouteTest {
    ...
    public void shouldCallCreateTheCalendarEventInOdoo() throws Exception {
        ...
        Mockito.when(mockXmlRpcClient.execute(mockXmlRpcClientConfig, ODOO_RPC_METHOD, rpcArgs))
                .thenReturn(expectedEventId);

        producerTemplate.send(URI_SAVE_CALENDAR_EVENT, exchange);

        assertEquals(expectedEventId, exchange.getIn().getBody());
    }
}
